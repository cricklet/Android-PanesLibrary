...
public class PanesLayout extends FrameLayout {
	...
	private PaneView getPaneFromScroll(double x) {
		int i = currentIndex;
		PaneView pane = null;
		double paneMinX = parentWidth;

		while (x < paneMinX && i >= 0 && i < panes.size()) {
			pane = panes.get(i);
			if (pane == null) return null;
			paneMinX -= pane.containerWidth;
			i --;
		}
	...
	public PaneView addPane(int type) {
		panesChanged = true;
		return addPane(type, false);
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (dx > 3 * dy && dx > 10) {
			...
			if (p != null && p.focused)
				return false;
			if (p != null) touchPaneWidth = p.containerWidth;
			else touchPaneWidth = parentWidth / 2;
			return true;
		}
